 ...
 */
 ...
 for (i=0; i<8; i++) {
    indiceNomeLista = Math.floor(Math.random() * listaNomi.length); 
    nomeLista = listaNomi[indiceNomeLista]
    
    indiceCognomeLista = Math.floor(Math.random() * listaNomi.length); 
    cognomeLista = listaCognomi[indiceCognomeLista]

    nomeIntero = nomeLista + ' ' + cognomeLista;

    console.log(nomeIntero);

    listaInvitati = document.getElementById("lista").innerHTML;

    document.getElementById("lista").innerHTML = listaInvitati + '<li>' + nomeIntero + '</li>';
 }
